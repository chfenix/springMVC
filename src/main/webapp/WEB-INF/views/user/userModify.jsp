		...
		<!-- 正文区域 -->
			...
			<!-- 上方提示框 -->
			    <div class="box box-info">
			        ...
			        <div class="box-body">
			            <form  action="modifyUserInfo.do" method="post" onsubmit="return userJudge();" id="user_form" name="user_form" class="form-horizontal bv-form" novalidate="novalidate">
			            	<!-- 隐藏域 -->
			                    ...
			                    <div class="row">
			                        ...
			                        <div class="col-md-6">
			                            ...
			                            <div class="form-group has-feedback">
			                                ...
			                                <div class="col-sm-8">
			                                    <tags:select selectId="status" selectName="status" typeCode="COMMON_VALID" selectValue="${ui.status}" selectStyle="width: 100%;height: 34px;padding: 6px 12px;border: 1px solid #d2d6de;"></tags:select>
			                                </div>
			                            </div>
...
function userJudge(){
	...
	//如果没有操作，则提示，并停留在当前页
	if(userName=='${ui.userName}'&&name=="${ui.name}"&&$("#password").val().trim()==""&&$("#mobile").val()=="${ui.mobile}"&&$("#email").val()=="${ui.email}"&&$("#status").val()=="${ui.status}"){
		if($("#roleIds").val()=='${roleIds}'){
			$("#alert_danger").css('display','block');
			$("#alert_danger").html("还未进行修改");
...
$(function(){
	...
	if("${respcode}"!=""){
	 ...
	 $('#user_form').bootstrapValidator({
        ...
        //    $('#user_form').bootstrapValidator('disableSubmitButtons', false);
        //},
        fields: {
       	 name:{
       		 trigger:"blur",
